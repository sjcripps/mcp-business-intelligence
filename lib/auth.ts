...
export async function validateApiKey(
  ...
  if (currentUsage >= limit) {
    return {
      valid: false,
      error: `Rate limit exceeded. ${keyData.tier} tier allows ${limit} requests/month. Current: ${currentUsage}. Upgrade at https://mcp.ezbizservices.com/pricing`,
    };
  }
...
export async function createApiKey(
  ...
  const store = await loadKeys();
  const key = `sk_biz_${randomBytes(24).toString("hex")}`;
  store.keys[key] = {
    name,
    email,
    tier,
...
export async function listApiKeys(): Promise<
  { key: string; name: string; tier: string; usage: number; active: boolean }[]
> {
  const store = await loadKeys();
  const month = new Date().toISOString().slice(0, 7);
  return Object.entries(store.keys).map(([key, data]) => ({
    key: key.slice(0, 12) + "..." + key.slice(-4),
    name: data.name,
    tier: data.tier,
    usage: data.usage[month] || 0,
    active: data.active,
  }));
}
